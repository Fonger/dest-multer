        ...
        promises.push(new Promise((resolve, reject) => {
          ...
          mkdirp(path.join(setting.root, _relDestFolder), (err) => {
            ...
            let _filename = setting.filename(req, file),
                _publicFilePath = path.join(_relDestFolder, _filename),
                _absFilePath = path.join(setting.root, _publicFilePath),
                _absDestination = path.join(setting.root, _relDestFolder);

	    var readStream = fs.createReadStream(file.path);
	    var writeStream = fs.createWriteStream(_absFilePath);
